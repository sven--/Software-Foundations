...
Definition member (v:nat) (s:bag) : bool := match ((count v) s) with
                                                | 0 => false
                                                         | S _ => true
                                                                    end.
...
(* "snoc" is reverse string of "cons" *)
...
Lemma elem_snoc_cons : forall b : nat, forall a : natlist,
snoc a b = a ++ [b].
Proof.
  simpl.
  induction a as [|h t].
...
Lemma rev_snoc_cons : forall a : nat, forall s : natlist,
rev (snoc s a) = a :: (rev s).
Proof.
intros a s.
rewrite -> (elem_snoc_cons a s).
induction s as [|h' t'].
simpl.
reflexivity.
simpl.
rewrite -> IHt'.
simpl.
rewrite -> elem_snoc_cons.
reflexivity.
(* a :: t' ++ [h'] = a :: h' :: t'
::, ++ is both level 60, but right assoc.
 *)
Qed.
...
Proof.
  ...
  reflexivity.
  simpl.
  rewrite -> rev_snoc_cons.
  rewrite -> IHt.
  reflexivity.
Qed.
(* length_snoc: forall (n : nat) (l : natlist), length (snoc l n) = S (length l) *)
...
Theorem my_thm : forall a c : natlist, forall d : nat,
(snoc a d) ++ c = a ++ [d] ++ c.
Proof.
  intros a b c.
  simpl.
...
Theorem bag_count_sum : forall n m : bag, forall v : nat,
                          (count v n) + (count v m) = (count v (sum n m)).
Proof.
  intros n m v.
  induction n as [|h t].
  simpl. reflexivity.
  simpl. destruct (beq_nat v h) as [t|f].
  simpl. rewrite IHt. reflexivity.
